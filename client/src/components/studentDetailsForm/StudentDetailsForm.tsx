...
const StudentDetailsForm: React.FC<StudentDetailsFormProps> = ({ componentUrl }) => {
    ...
    const onSave = async (values: StudentForm) => {
        ...
        for (const job of jobsAndEmployees) {
            const existingJobId = await checkExistingJob(job.job);
            if (existingJobId === 0) {
                ...
                const employeeName = jobForEmployee.find(emp => emp.employee_id === job.employee)?.name || 'Unknown Employee';
                addMessage(`התפקיד לא נשמר עבור העובד ${employeeName}`, "error");
            }
        }
    ...
    const checkExistingJob = async (jobId: number) => {
        const year = await getYearForSystem();
        const employess = await studentService.checkExistingJob(studentId!, year, jobId);
        if (employess.exitingEmployees[0].length > 0) {
            return jobId;
        } else {
            return 0;
        }
    ...
    const handleRemoveEmployee = async (employeeId: string) => {
        ...
        const employees = await checkExistingJob(jobId!);
        if (employees === 0) {
